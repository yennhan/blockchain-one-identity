...
def supplier_homepage():
    ...
    for i in range(len(the_restock)):
        if the_restock[i]['supplierID'] == "resource:org.acme.model.supplier#supplier1" and the_restock[i]['restock_status'] == "Required_restock":
            data_invoice.append(the_restock[i])
    return render_template('supplier1_dashboard.html', invoice=data_invoice)
...
def issue_invoice():
    if request.method == 'POST':
        ...
        the_items = []
        data = {}
        data['qty'] = (int((the_list['standard_level'] * 1.5)))
